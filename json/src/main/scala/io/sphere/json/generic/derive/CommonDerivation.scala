...
object CommonDerivation extends Logging {
  ...
  private[derive] val defaultTypeFieldName: String = JSONTypeHintField2.defaultValue
  ...
  private def getJSONFieldMeta(caseClass: CaseClass[Typeclass, _]): IndexedSeq[JSONFieldMeta] = {
    caseClass.parameters.map { p =>
      ...
      val embedded = annotations.exists {
        case _: JSONEmbedded2 => true
        case _ => false
      }
      val ignored = annotations.exists {
        case _: JSONIgnore2 => true
        case _ => false
      }
  ...
  private def typeHint(typeName: TypeName, annotations: Seq[Any]): Option[JSONClassMeta.TypeHint] = {
    def hintVal(h: JSONTypeHint2): String =
      if (h.value.isEmpty) defaultTypeValue(typeName)
      else h.value

    val typeHintFieldAnnot: Option[JSONTypeHintField2] = annotations.collectFirst {
      case h: JSONTypeHintField2 => h
    }
    val typeHintAnnot: Option[JSONTypeHint2] = annotations.collectFirst {
      case h: JSONTypeHint2 => h
    }
