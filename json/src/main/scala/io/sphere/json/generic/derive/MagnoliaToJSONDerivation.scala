...
object MagnoliaToJSONDerivation extends CommonDerivation {
  ...
  def dispatch[T](sealedTrait: SealedTrait[ToJSON, T]): ToJSON[T] = new ToJSON[T] {
    ...
    private val typeField = sealedTrait.annotations.collectFirst {
      case a: JSONTypeHintField2 => a.value
    ...
    override def write(t: T): JValue = {
      sealedTrait.dispatch(t) { subtype =>
        writeMap.get(subtype.typeName) match {
          ...
          case Some(ts) => subtype.typeclass.write(subtype.cast(t)) match {
