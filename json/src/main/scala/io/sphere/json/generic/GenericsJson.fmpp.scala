...
trait GenericsJson {
  ...
  protected def JSONofToAndFrom[A](toJSON: ToJSON[A], fromJSON: FromJSON[A]): JSON[A] = {
    new JSON[A] {
  ...
  protected def jsonSingletonTypeValue[T](singleton: T): String = {
  ...
  protected def jsonProduct0Type[T <: Product](singleton: T): (String, String) = {
  ...
  protected def typeSelectorToJSON[A: ClassTag: ToJSON](): TypeSelectorToJSON[_] = {
  ...
  protected def typeSelectorFromJSON[A: ClassTag: FromJSON](): TypeSelectorFromJSON[_] = {
    val clazz = classTag[A].runtimeClass
    val (typeField, typeValue) = getJSONClass(clazz).typeHint match {
      case Some(hint) => (hint.field, hint.value)
      case None => (defaultTypeFieldName, defaultTypeValue(clazz))
  ...
  protected def typeSelector[A: ClassTag: FromJSON: ToJSON](): TypeSelector[_] = {
    val clazz = classTag[A].runtimeClass
    val (typeField, typeValue) = getJSONClass(clazz).typeHint match {
      case Some(hint) => (hint.field, hint.value)
      case None => (defaultTypeFieldName, defaultTypeValue(clazz))
  ...
  protected def defaultTypeValue(clazz: Class[_]): String =
    clazz.getSimpleName.replace("$", "")

  protected def findTypeValue(o: JObject, typeField: String): Option[String] =
  ...
  protected val getJSONClass = new Memoizer[Class[_], JSONClassMeta](clazz => {
  ...
  protected case class JSONClassMeta(typeHint: Option[JSONClassMeta.TypeHint], fields: IndexedSeq[JSONFieldMeta])
  protected object JSONClassMeta {
    case class TypeHint(field: String, value: String)
  }
  protected case class JSONFieldMeta(
                                    name: String,
                                    default: Option[Any] = None,
                                    embedded: Boolean = false,
                                    ignored: Boolean = false)

  protected def getJSONFields(clazz: Class[_]): IndexedSeq[JSONFieldMeta] = {
  ...
  protected def writeField[A: ToJSON](buf: ListBuffer[JField], field: JSONFieldMeta, e: A) {
    if (!field.ignored) {
      if (field.embedded)
        toJValue(e) match {
  ...
  protected def writeTypeField(jClass: JSONClassMeta, buf: ListBuffer[JField]): Unit =
    jClass.typeHint foreach { th =>
      buf += JField(th.field, JString(th.value))
    }

  protected def readField[A: FromJSON](f: JSONFieldMeta, o: JObject): JSONParseResult[A] = {
