...
abstract class MagnoliaUtils {
  ...
  case class TypeSelector[A](
      val typeField: String,
      val typeValue: String,
      subType: Subtype[JSON, A])

  def typeSelector[A](subType: Subtype[JSON, A]): TypeSelector[A] = {
  ...
  def fieldMeta(p: Param[JSON, _]): JSONFieldMeta = {
  ...
  def getJSONClassMeta = new Memoizer[CaseClass[JSON, _], JSONClassMeta](caseClass => {
  ...
  val defaultTypeFieldName: String = JSONTypeHintField.defaultValue

  def defaultTypeValue(typeName: TypeName): String =
    typeName.short.replace("$", "")

  def hintVal(typeName: TypeName)(h: JSONTypeHint): String =
    if (h.value.trim.isEmpty) defaultTypeValue(typeName)
    else h.value

  case class JSONClassMeta(
      typeHint: Option[JSONClassMeta.TypeHint],
      fields: IndexedSeq[JSONFieldMeta])
  object JSONClassMeta {
    case class TypeHint(field: String, value: String)
  }
  case class JSONFieldMeta(
      name: String,
      default: Option[Any] = None,
      embedded: Boolean = false,
      ignored: Boolean = false)

  def readField[A: FromJSON](f: JSONFieldMeta, o: JObject): JSONParseResult[A] = {
