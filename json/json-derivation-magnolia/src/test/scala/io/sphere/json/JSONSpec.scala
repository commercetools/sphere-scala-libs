...
import io.sphere.json.generic.derive.FromJSONDerivation.fromJsonProduct
import io.sphere.json.generic.derive.ToJSONDerivation.toJsonProduct
...
class JSONSpec extends AnyFunSpec with Matchers {
  ...
  describe("JSON") {
    ...
    it("must provide derived instances for singleton objects") {
      // TODO: YaSi: fix
//      implicit val singletonJSON: JSON[Singleton] = deriveJSON[Singleton.type]
//      val json = s"""[${toJSON(Singleton)(singletonJSON)}]"""
//      withClue(json) {
...
//    }
  ...
  describe("ToJSON and FromJSON") {
    it("must provide derived JSON instances for sum types") {
      // ToJSON
      implicit val birdToJSON = toJsonProduct[Bird]
      implicit val dogToJSON = toJsonProduct[Dog]
      implicit val catToJSON = toJsonProduct[Cat]
      implicit val animalToJSON = toJsonProduct[Animal]
      // FromJSON
      implicit val birdFromJSON = fromJsonProduct[Bird]
      implicit val dogFromJSON = fromJsonProduct[Dog]
      implicit val catFromJSON = fromJsonProduct[Cat]
      implicit val animalFromJSON = fromJsonProduct[Animal]
    ...
    it("must provide derived instances for product types with concrete type parameters") {
      implicit val aToJSON = toJsonProduct[GenericA[String]]
      implicit val aFromJSON = fromJsonProduct[GenericA[String]]
      val a = GenericA("hello")
      fromJSON[GenericA[String]](toJSON(a)) must equal (Valid(a))
    }
    ...
    it("must provide derived instances for singleton objects") {
      // TODO: YaSi: fix
//      implicit val toSingletonJSON = toJsonProduct[Singleton.type]
//      implicit val fromSingletonJSON = fromJsonProduct[Singleton.type]
...
//      }
      ...
      // ToJSON
      implicit val toSingleAJSON = toJsonProduct[SingletonA.type]
      implicit val toSingleBJSON = toJsonProduct[SingletonB.type]
      implicit val toSingleCJSON = toJsonProduct[SingletonC.type]
      implicit val toSingleEnumJSON = toJsonProduct[SingletonEnum]
      // FromJSON
      implicit val fromSingleAJSON = fromJsonProduct[SingletonA.type]
      implicit val fromSingleBJSON = fromJsonProduct[SingletonB.type]
      implicit val fromSingleCJSON = fromJsonProduct[SingletonC.type]
      implicit val fromSingleEnumJSON = fromJsonProduct[SingletonEnum]
    ...
    it("must provide derived instances for sum types with a mix of case class / object") {
      // ToJSON
      implicit val toSingleJSON = toJsonProduct[SingletonMixed.type]
      implicit val toRecordJSON = toJsonProduct[RecordMixed]
      implicit val toMixedJSON = toJsonProduct[Mixed]
      // FromJSON
      implicit val fromSingleJSON = fromJsonProduct[SingletonMixed.type]
      implicit val fromRecordJSON = fromJsonProduct[RecordMixed]
      implicit val fromMixedJSON = fromJsonProduct[Mixed]
      List(SingletonMixed, RecordMixed(1)) foreach { m: Mixed =>
        fromJSON[Mixed](toJSON(m)) must equal (Valid(m))
      }
    }
    ...
    it("must handle subclasses correctly in `jsonTypeSwitch`") {
      // ToJSON
      implicit val to1 = toJsonProduct[TestSubjectConcrete1]
      implicit val to2 = toJsonProduct[TestSubjectConcrete2]
      implicit val to3 = toJsonProduct[TestSubjectConcrete3]
      implicit val to4 = toJsonProduct[TestSubjectConcrete4]
      implicit val toA = toJsonProduct[TestSubjectCategoryA]
      implicit val toB = toJsonProduct[TestSubjectCategoryB]
      implicit val toBase = toJsonProduct[TestSubjectBase]

      // FromJSON
      implicit val from1 = fromJsonProduct[TestSubjectConcrete1]
      implicit val from2 = fromJsonProduct[TestSubjectConcrete2]
      implicit val from3 = fromJsonProduct[TestSubjectConcrete3]
      implicit val from4 = fromJsonProduct[TestSubjectConcrete4]
      implicit val fromA = fromJsonProduct[TestSubjectCategoryA]
      implicit val fromB = fromJsonProduct[TestSubjectCategoryB]
      implicit val fromBase = fromJsonProduct[TestSubjectBase]
    ...
    it("must provide derived JSON instances for product types (case classes)") {
      import JSONSpec.{ Project, Milestone }
      // ToJSON
      implicit val milestoneToJSON = toJsonProduct[Milestone]
      implicit val projectToJSON = toJsonProduct[Project]
      // FromJSON
      implicit val milestoneFromJSON = fromJsonProduct[Milestone]
      implicit val projectFromJSON = fromJsonProduct[Project]
...
sealed abstract class TestSubjectBase
