...
trait DeriveFromJSON {
  ...
  protected object Derivation {
    ...
    inline def derived[A](using m: Mirror.Of[A]): FromJSON[A] =
      inline m match {
        case s: Mirror.SumOf[A] => fromJsonTypeSwitch[A](subsFrom[s.MirroredElemTypes])
    ...
    inline private def deriveCaseClass[A](mirrorOfProduct: Mirror.ProductOf[A]): FromJSON[A] = {
      ...
      def readField(field: Field, fromJson: FromJSON[Any], jObject: JObject): JValidation[Any] =
        if (field.ignored) {
          // 1. Ignored fields are never read from JSON (present or not); always load the default.
          // 2. The default is guaranteed to exist — validated above.
          Validated.valid(field.defaultArgument.get)
