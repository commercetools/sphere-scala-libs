...
class ValidatedNelTestOpsSpec extends AnyFunSpec with Matchers {
  ...
  describe("expectValid") {
    ...
    it("should throw IllegalStateException when Invalid") {
      val result = Invalid(NonEmptyList.one("error"))
      val ex = the[IllegalStateException] thrownBy result.expectValid
  ...
  describe("expectErrors") {
    ...
    it("should throw IllegalStateException when Valid") {
      val result = Valid(42)
      val ex = the[IllegalStateException] thrownBy result.expectErrors
  ...
  describe("expectError") {
    ...
    it("should throw IllegalStateException when Valid") {
      val result = Valid(42)
      val ex = the[IllegalStateException] thrownBy result.expectError
      ex.getMessage must be("Expected Invalid, but got: 42")
    }
