...
class DeriveMongoFormatSpec extends AnyWordSpec with Matchers {
  ...
  "deriving MongoFormat" must {
    ...
    "fail to derive if trait is not sealed" in {
      // Sealed
      "implicit val mongo: TypedMongoFormat[SealedSub] = deriveMongoFormat[SealedSub]" must compile
      // Not sealed
      "implicit val mongo: TypedMongoFormat[NotSealed] = deriveMongoFormat[NotSealed]" mustNot compile
      // Sealed, but child is not sealed
      "implicit val mongo: TypedMongoFormat[SealedParent] = deriveMongoFormat[SealedParent]" mustNot compile
    }
  }
}
...
object DeriveMongoFormatSpec {
  ...
  given TypedMongoFormat[PictureSize] = deriveMongoFormat[PictureSize]
  ...
  given TypedMongoFormat[Access] = deriveMongoFormat
  ...
  given TypedMongoFormat[UserWithPicture] = deriveMongoFormat
